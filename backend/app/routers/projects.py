...
from app.services import s3_service, sfn_service, dataset_service
...
async def get_upload_url(project_id: str, filename: str = Query(...)):
    """Generate a presigned S3 PUT URL for file upload."""
    project = db.get_project(project_id)
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")

    result = s3_service.generate_presigned_upload_url(project_id, filename)
    return result
...
async def start_pipeline(project_id: str, req: StartPipelineRequest):
    ...
    try:
        ...
        logger.info("Started pipeline for project %s: %s", project_id, execution_arn)
        return {
        ...
        logger.error("Failed to start pipeline for project %s: %s", project_id, e)
