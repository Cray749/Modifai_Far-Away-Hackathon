...
Uses Gemini as a critic agent to estimate test performance from training
metrics.  No Amazon Bedrock dependency.

Scoring
-------
metric_score   = max(0, 1 - training_loss)      — derived from training loss
test_score     = Gemini estimate                 — critic-agent judgement
...
def lambda_handler(event: dict, context) -> dict:
    ...
    --------------------
    {
      ...
      "job_info": {                          # optional — enriches Gemini prompt
    ...
    # Optional context to help Gemini give a more grounded estimate
    ...
    try:
        eval_data  = call_gemini_json(prompt=prompt, system=_SYSTEM_PROMPT)
        test_score = float(eval_data.get("test_score", 0.5))
        confidence = float(eval_data.get("confidence", 0.5))
        reasoning  = eval_data.get("reasoning", "")
        logger.info(
            "Gemini evaluation: test_score=%.4f, confidence=%.4f, reasoning=%s",
            test_score, confidence, reasoning,
        )
    except Exception as exc:  # noqa: BLE001
        logger.error("Gemini evaluation failed — using fallback test_score=0.75: %s", exc)
        test_score = 0.75
        confidence = 0.5
        reasoning  = "Fallback score used due to Gemini evaluation error."
